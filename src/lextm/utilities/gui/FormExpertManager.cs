...
namespace Lextm.Utilities.Gui
{
    ...
    public class FormExpertManager : Form
    {
        ...
        private void btActivate_Click(object sender, System.EventArgs e)
        {
            ...
            if (_ExpertInfo != null)
            {
                _ExpertInfo.Activate();
                RefreshExpertAssemblies();
            }
        ...
        private void btAdd_Click(object sender, System.EventArgs e)
        {
            if (openFileDialog.ShowDialog() == DialogResult.OK)
            {
                ...
                if (ExpertInfoExists(fileName))
                {
                    ValidationHelpers.ShowWarning("The selected IDE Assembly is already registered !");
                }
                if (Lextm.IO.FileHelper.IsDotNetAssembly(fileName))
                {
                    ExpertInfo.Add(fileName, Path.GetFileName(fileName), IDEVersion);
                }
                else
                {
                    ExpertInfo.Add(fileName, Path.GetFileName(fileName), IDEVersion, false);
                }
                RefreshExpertAssemblies();
            }
        ...
        private void btDelete_Click(object sender, System.EventArgs e)
        {
            ...
            if (_ExpertInfo != null && Lextm.Windows.Forms.MessageBoxFactory.Confirm("Do you really want to remove this IDE assembly ?") == DialogResult.Yes)
            {
                ...
                {
                    ExpertInfo.Delete(_ExpertInfo.IsActive, _ExpertInfo.FileName, IDEVersion);
                }
                else
                {
                    ExpertInfo.Delete(_ExpertInfo.IsActive, _ExpertInfo.FileName, IDEVersion, false);
                }
                RefreshExpertAssemblies();
            }
        ...
        private void EnableControls()
        {
            ...
            tsbtnDelete.Enabled = (_ExpertInfo != null);
            tsbtnDisable.Enabled = ((_ExpertInfo != null) && (_ExpertInfo.IsActive));
            tsbtnEnable.Enabled = ((_ExpertInfo != null) && (!_ExpertInfo.IsActive));
        }

        private bool ExpertInfoExists(string aFileName)
        {
            foreach (ExpertInfo _ExpertInfo in fExpertInfoList)
            {
                if (_ExpertInfo.FileName == aFileName)
                {
                    return true;
                }
            }
        ...
        private ExpertInfo GetCurrentExpertInfo()
        {
            if (lvExperts.SelectedItems.Count > 0)
            {
                return (ExpertInfo)lvExperts.SelectedItems[0].Tag;
            }
            else
            {
                return null;
        ...
        private void LoadExpertInfo()
        {
            ...
            fExpertInfoList.Clear();

            RegistryKey _RegistryKey = null;

            string[] _ValueNames;
            LoggingService.Info("IDE version is " + IDEVersion);
            // Load Enabled
            _RegistryKey = Registry.CurrentUser.OpenSubKey(OtaUtils.GetEnabledAssembliesRegKey(IDEVersion));

            if (_RegistryKey != null)
            {
                _ValueNames = _RegistryKey.GetValueNames();

                foreach (string _ValueName in _ValueNames)
                {
                    try
                    {
                        ExpertInfo _ExpertInfo = new ExpertInfo(_RegistryKey.GetValue(_ValueName).ToString(),
                                                                _ValueName,
                                                                true,
                                                                IDEVersion);
                        fExpertInfoList.Add(_ExpertInfo);
                    }
                    catch
                    {
            ...
            // Load Disabled
            _RegistryKey = Registry.CurrentUser.OpenSubKey(OtaUtils.GetDisabledAssembliesRegKey(IDEVersion));
            ...
            // Win32
            // Load Enabled
            _RegistryKey = Registry.CurrentUser.OpenSubKey(OtaUtils.GetEnabledPackagesRegKey(IDEVersion));
            ...
            // Load Disabled
            _RegistryKey = Registry.CurrentUser.OpenSubKey(OtaUtils.GetDisabledPackagesRegKey(IDEVersion));
        ...
        private void LoadExpertAssembliesInListView()
        {

            LoggingService.EnterMethod();
            ExpertInfo _SelectedExpertInfo = null;
            try
            {
                // Get the Selected Items
                if (lvExperts.SelectedItems.Count == 1)
                {
                    _SelectedExpertInfo = (ExpertInfo)lvExperts.SelectedItems[0].Tag;
                }
                ...
                foreach (ExpertInfo _ExpertInfo in fExpertInfoList)
                {
                    ...
                    _ListViewItem.Text = "[" + _ExpertInfo.IsActive.ToString() + "]";
                    if (_ExpertInfo.IsDotNet)
                    {
                        _ListViewItem.SubItems.Add(_ExpertInfo.Name);
                        _ListViewItem.SubItems.Add(_ExpertInfo.FileName);
                    }
                    else
                    {
                        _ListViewItem.SubItems.Add(_ExpertInfo.FileName);
                    ...
                    _ListViewItem.Tag = _ExpertInfo;

                    lvExperts.Items.Add(_ListViewItem);
                }
            }
            finally
            {
                if (_SelectedExpertInfo != null)
                {
                    foreach (ListViewItem _Item in lvExperts.Items)
                    {
                        if ((_Item.Tag as ExpertInfo).FileName == _SelectedExpertInfo.FileName)
                        {
                            _Item.Selected = true;
                            break;
                        }
        ...
        private void RefreshExpertAssemblies()
        {
            LoggingService.EnterMethod();

            LoadExpertInfo();
            LoadExpertAssembliesInListView();
            EnableControls();
